...
if(isset($_GET['action']) && $_GET['action']==1){
  ...
  $arquivo= $_FILES['doc'];
  $nome= time().'.jpg';
  move_uploaded_file($arquivo['tmp_name'], '../images/'.$nome);

		if(!empty($arquivo)){
			$sel = $db->select("INSERT INTO banner (urli) VALUES ('$nome')");
...
//FAZ A PESQUISA PARA ALTERAÇAO NO BANCO DE DADOS
	...
	$sel = $db->select("SELECT * FROM banner WHERE id='$id' LIMIT 1");
	$ln = $db->expand($sel);
...
//FAZ A exclusao NO BANCO DE DADOS
	...
	$sel = $db->select("DELETE FROM banner WHERE id='$id' LIMIT 1");
	echo '<script>$("#sucesso_user").show();</script>';
...
//FAZ A ALTERAÇAO NO BANCO DE DADOS
  ...
  move_uploaded_file($arquivo['tmp_name'], '../documentos/'.$nome);
		...
		if(!empty($arquivo)){
			$sel = $db->select("UPDATE banner SET urli='$nome' WHERE id='$id' LIMIT 1");
			echo '<script>$("#sucesso_user").show();</script>';
		}
		echo '<script>$("#sucesso_user").show();</script>';
    ...
    <div class="col-md-12">
        ...
        <div class="col-md-4">
           <div class="form-group">
                <label for="exampleInputEmail1">Banner <small>(1260 x 625)</small><span style="color: red;"> **</span>  </label>
...
<div class="panel panel-primary">
	...
	<div class="panel-heading">
		<h4 class="panel-title"><h4>Cadastro de fotos</h4></h4>
	</div>
...
<div class="row">
	...
	<?php
		$hoje=date("Y-m-d");
		$db = new DB();
		$sel = $db->select("SELECT * FROM banner ORDER BY id DESC");
		if($db->rows($sel)){
			$x=1;
            ...
            <img src="../images/'.$yy['urli'].'" class="img-responsive" />
