...
if(isset($_GET['action']) && $_GET['action']==1){
  ...
  move_uploaded_file($arquivo['tmp_name'], '../images/'.$nome);
		...
		if(!empty($arquivo)){
			$sel = $db->select("INSERT INTO galery (urli) VALUES ('$nome')");
...
//FAZ A PESQUISA PARA ALTERAÇAO NO BANCO DE DADOS
	...
	$sel = $db->select("SELECT * FROM galery WHERE id='$id' LIMIT 1");
	$ln = $db->expand($sel);
...
//FAZ A exclusao NO BANCO DE DADOS
	...
	$sel = $db->select("DELETE FROM galery WHERE id='$id' LIMIT 1");
	echo '<script>$("#sucesso_user").show();</script>';
...
//FAZ A ALTERAÇAO NO BANCO DE DADOS
  ...
  move_uploaded_file($arquivo['tmp_name'], '../documentos/'.$nome);
		...
		if(!empty($arquivo)){
			$sel = $db->select("UPDATE galery SET urli='$nome' WHERE id='$id' LIMIT 1");
			echo '<script>$("#sucesso_user").show();</script>';
		}
		echo '<script>$("#sucesso_user").show();</script>';
    ...
    <div class="col-md-12">
        ...
        <div class="col-md-4">
           <div class="form-group">
                <label for="exampleInputEmail1">Documento <small>(1140 x 500)</small><span style="color: red;"> **</span>  </label>
