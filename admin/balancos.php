...
if(isset($_GET['action']) && $_GET['action']==1){
  ...
  move_uploaded_file($arquivo['tmp_name'], '../documentos/'.$nome);
		...
		if(!empty($arquivo)){
			$sel = $db->select("INSERT INTO balancos (titulo,urli,ano) VALUES ('$titulo','$nome','$ano')");
...
//FAZ A PESQUISA PARA ALTERAÇAO NO BANCO DE DADOS
	...
	$sel = $db->select("SELECT * FROM balancos WHERE id='$id' LIMIT 1");
	$ln = $db->expand($sel);
...
//FAZ A exclusao NO BANCO DE DADOS
	...
	$sel = $db->select("DELETE FROM balancos WHERE id='$id' LIMIT 1");
	echo '<script>$("#sucesso_user").show();</script>';
...
//FAZ A ALTERAÇAO NO BANCO DE DADOS
  ...
  move_uploaded_file($arquivo['tmp_name'], '../documentos/'.$nome);
		...
		if(!empty($arquivo)){
			$sel = $db->select("UPDATE balancos SET titulo='$titulo',urli='$nome',ano='$ano' WHERE id='$id' LIMIT 1");
			echo '<script>$("#sucesso_user").show();</script>';
		}
		echo '<script>$("#sucesso_user").show();</script>';
...
<div class="row">
	...
	<?php
		$hoje=date("Y-m-d");
		$db = new DB();
		$sel = $db->select("SELECT * FROM balancos ORDER BY id DESC");
		if($db->rows($sel)){
			$x=1;
